...
<br/><br/>
Authenticate service is an [[https://docs.microsoft.com/en-us/aspnet/web-api/overview/getting-started-with-aspnet-web-api/tutorial-your-first-web-api|Web API]] service at all, which controls another services authorization safety with [[https://identityserver.github.io/Documentation|IdentityServer3]] project. IdentityServer also is an [[http://openid.net/connect|OpenID Connect]] Provider and [[https://www.digitalocean.com/community/tutorials/an-introduction-to-oauth-2|OAuth 2.0]] Authorization Server Framework for ASP.NET 4.x/Katana.
So, first you need to know what is the [[http://openid.net/connect|OpenID Connect]] protocol ?

<hr/>
<br/>

## [[http://openid.net/connect|OpenID Connect]] Protocol

    OpenID Connect is a simple identity layer on top of the [[https://www.digitalocean.com/community/tutorials/an-introduction-to-oauth-2|OAuth 2.0]]  protocol. It allows Clients to verify the identity of the End-User based on the authentication performed by an Authorization Server, as well as to obtain basic profile information about the End-User in an inter-operable and REST-like manner.

OpenID Connect allows clients of all types, including Web-based, mobile, and JavaScript clients, to request and receive information about authenticated sessions and end-users. The specification suite is extensible, allowing participants to use optional features such as encryption of identity data, discovery of OpenID Providers, and session management, when it makes sense for them. OpenID Connect defines optional mechanisms for robust signing and encryption. Whereas integration of OAuth 1.0a and OpenID 2.0 required an extension, in OpenID Connect, [[https://www.digitalocean.com/community/tutorials/an-introduction-to-oauth-2|OAuth 2.0]]   capabilities are integrated with the protocol itself.
...
[[File:/authentication service/images/OpenIDConnect-Map-4Feb2014.png|]]

For more information go to [[http://openid.net/connect|reference]]

<hr/>
<br/>

## [[https://www.digitalocean.com/community/tutorials/an-introduction-to-oauth-2|Introduction to OAuth 2]]
...
<blockquote>
* <code>Resource Owner: User</code> <br/>
    <blockquote>The resource owner is the user who authorizes an application to access their account. The application's access to the user's account is limited to the "scope" of the authorization granted (e.g. read or write access).</blockquote>
* <code>Client: API</code>  <br/>
    <blockquote>The client is the application that wants to access the user's account. Before it may do so, it must be authorized by the user, and the authorization must be validated by the API.</blockquote>
* <code>Resource Server: API</code>  <br/>
    <blockquote>The resource server hosts the protected user accounts.</blockquote>
* <code>Authorization Server: Application</code>  <br/>
    <blockquote>The authorization server verifies the identity of the user then issues access tokens to the application.</blockquote>

[[File:/authentication service/images/abstract_flow.png|]]
</blockquote>
...
<br/>
### __Client ID and Client Secret__
<blockquote>
...
* <code>__Authorization Code__: used with server-side Applications</code>  <br/>
    <blockquote>The __authorization code__ grant type is the most commonly used because it is optimized for server-side applications, where source code is not publicly exposed, and Client Secret confidentiality can be maintained. This is a redirection-based flow, which means that the application must be capable of interacting with the user-agent (i.e. the user's web browser) and receiving API authorization codes that are routed through the user-agent. 

[[File:/authentication service/images/auth_code_flow.png|]]
</blockquote>
* <code>__Implicit__: used with Mobile Apps or Web Applications (applications that run on 
the user's device)</code>  <br/>
<blockquote>
...
The implicit grant type does not support refresh tokens. <br/><br/>
...
[[File:/authentication service/images/implicit_flow.png|]]
</blockquote>
* <code>__Resource Owner Password Credentials__: used with trusted Applications, such as those owned by the service itself</code>  <br/>
<blockquote>
...
* <code>__Client Credentials__: used with Applications API access</code>  <br/>
<blockquote>
...
For more information go to [[https://www.digitalocean.com/community/tutorials/an-introduction-to-oauth-2|reference]]

<hr/>
<br/>

## [[https://github.com/IdentityServer/IdentityServer3|Identity Server 3]]
...
[[File:/authentication service/images/appArch.png|]]
...
[[File:/authentication service/images/protocols.png|]]
...
[[File:/authentication service/images/terminology.png|]]
...
<br/><br/>
Different literature uses different terms for the same role - you probably also find security token service, identity provider, authorization server, <code>IP-STS</code> and more.
<br/><br/>
...
<br/><br/>
There is e.g. a scope called <code>profile</code> that includes first name, last name, preferred username, gender, profile picture and more. You can read about the standard scopes [[http://openid.net/specs/openid-connect-core-1_0.html#ScopeClaims|here]] and you can create your own scopes in IdentityServer to model your own requirements.
<br/><br/>
*__Resource scopes__<br/>
Resource scopes identify web APIs (also called resource servers) - you could have e.g. a scope named <code>calendar</code> that represents your calendar API.
</blockquote>

####__Claims__
<blockquote>
List of user claims that should be included in the identity (identity scope) or access token (resource scope). 
Scope can also specify claims that go into the corresponding token - the <code>ScopeClaim</code> class has the following properties:
*Name
**Name of the claim
*Description
**Description of the claim
*AlwaysIncludeInIdToken
**Specifies whether this claim should always be present in the identity token (even if an access token has been requested as well). Applies to identity scopes only. Defaults to <code>false</code>.
<br/><br/>

Example of a <code>role</code> identity scope:
<br/><br/>
```cs
...
For more information go to the [[https://identityserver.github.io/Documentation/docsv2|documentation for the latest version]]
...
#### Introduction to [[https://www.digitalocean.com/community/tutorials/an-introduction-to-oauth-2#grant-type-client-credentials|OAuth 2 Client Credential Grant Type]]

<blockquote>
This project use [[https://github.com/IdentityServer/IdentityServer3|IdentityServer3]] by OAuth __client credential__ grant types for authenticate all clients as API or end point clients. <br/>
...
<br/>
IdentityServer publishes a [[http://localhost:port/.well-known/openid-configuration|discovery document]] where you can find metadata and links to all the endpoints, key material, etc, like the below data:
<br/><br/>
```
...

{
  ...
  "jwks_uri": "http://localhost:port/.well-known/jwks",
  "authorization_endpoint": "http://localhost:port/connect/authorize",
  "token_endpoint": "http://localhost:port/connect/token",
  "userinfo_endpoint": "http://localhost:port/connect/userinfo",
  "end_session_endpoint": "http://localhost:port/connect/endsession",
  "check_session_iframe": "http://localhost:port/connect/checksession",
  "revocation_endpoint": "http://localhost:port/connect/revocation",
  "introspection_endpoint": "http://localhost:port/connect/introspect",
...
```
<br/>
In the [[http://localhost:port/.well-known/openid-configuration|IdentityServer meta data documentation]] you can see the authentication server addresses and meta data like __token request__ address by name "[[http://localhost:port/connect/token|token_endpoint]]" or supported scopes name by name "__scopes_supported__" or public key and other security data by name __jwks_uri__ and etc.
<br/><br/>
*__Client Credentials Flow__<br/>
...
<br/><br/>
An access token is a data of [[https://jwt.io|JWT (JSON Web Token)]] that you can to open or validate it from reference web site's for manually decoding.
[[https://jwt.io|JWT (JSON Web Tokens)]] are an open, industry standard RFC 7519 method for representing claims securely between two parties.
<br/>
[[[https://jwt.io|JWT.IO]] allows you to decode, verify and generate JWT.
<br/><br/>
__What is the JSON Web Token structure?__
<blockquote>
JSON Web Tokens consist of three parts separated by dots (<code>.</code>), which are:
<br/>
<blockquote>
**Header
**Payload
...
```
<br/>
Then, this JSON is __[[https://www.base64decode.org|Base64 Url]]__ encoded to form the first part of the JWT.
<br/><br/><br/>
__Payload__
<br/><br/>
...
<br/><br/>
<pre>Notice that the claim names are only three characters long as JWT is meant to be compact.</pre>
<br/>
...
```
{
  ...
  "aud": "http://localhost:port/resources",
  "exp": 1494313179,
  "nbf": 1494309579,
  "client_id": "admin",
  "role": [
...
```
<br/>
The payload is then __[[https://www.base64decode.org|Base64 Url]]__ encoded to form the second part of the JSON Web Token.
<br/><br/><br/>
__Signature__
<br/><br/>
...
<br/><br/>
[[File:/authentication service/images/encoded-jwt3.png|]]
<br/>
If you want to play with JWT and put these concepts into practice, you can use [[http://jwt.io|jwt.io Debugger]] to decode, verify, and generate JWTs.
<br/>
[[File:/authentication service/images/legacy-app-auth-5.png|]]
<br/><br/>
...
<br/><br/>
[[File:/authentication service/images/jwt-diagram.png|]]

</blockquote>

</blockquote>
<br/><br/>
Here is an example of an __API__ request, using <code>curl</code>. Note that it includes the access token:
<br/><br/>
<code>curl -X POST -H 'Authorization: Bearer ACCESS_TOKEN' 'https://localhost:port/v1/$OBJECT'</code>
<br/><br/>
...
<br/><br/>
*__Access Token Validation__<br/>
<blockquote>
...
<br/>
We can to validate access token without local checking and use self authentication server instead that. For this job's we must post the __AccessToken__ to one of authentication server API's methods by name __accesstokenvalidation__ like following:
<br/><br/>
```
POST            http://localhost:port/connect/accesstokenvalidation
Content-Type:   application/x-www-form-urlencoded
Body:   
{
...
```
<br/>
If the __AccessToken__ is valid then response message is like this:
<br/><br/>
```
{
  "iss": "http://localhost:port",
...
<br/><br/>
Here is an example __POST__ request, using a refresh token to obtain a new access token:
<br/><br/>
<code>https://localhost:port/v1/oauth/token?grant_type=__refresh_token__&client_id=__CLIENT_ID__&client_secret=__CLIENT_SECRET__&refresh_token=__REFRESH_TOKEN__</code>
<br/><br/>
</blockquote>
</blockquote>

* <code>IdentityServer</code>
<blockquote>
Identity server placed in [[http://localhost:port]]. It is based on [[https://github.com/IdentityServer/IdentityServer3|identity server 3]]. It implemented Oauth 2 and [[http://openid.net/connect|OpenId connect protocols ]].
Identity server has a console for managing clients and scopes. It can only be reached on localhost (remote localhost:port server). All data on identity server is persisted on sql server.
<br/>
IdentityServer publishes a [[http://localhost:port//.well-known/openid-configuration|discovery document]] where you can find metadata and links to all the endpoints, key material, etc.
<br/><br/>
...
<br/><br/>
**__Setting up IdentityServer__<br/>
...
APIs are modeled as scopes - you need to register all APIs that you want to be able to request access tokens for. For that we create a class that returns a list of <code>Scope</code> at __Config/Scopes.cs__: <br/><br/>
<blockquote>
```cs
...
***Access to the <code>api1</code> scope
<br/><br/>
...
<blockquote>
<br/>
<code>Note:</code> If the client use __ClientCredentials__ flow then must be set the __SigningCertificate__ property of __IdentityServerOptions__ instance at identity server __Startup.cs__ file's to __private_key_file.pfx__ files.
<br/><br/>
*__Configuring IdentityServer__<br/>
IdentityServer is implemented as OWIN middleware. It is configured in the <code>Startup</code> class using the <code>UseIdentityServer</code> extension method. The following snippets sets up a bare bones server with our scopes and clients. We also set up an empty list of users - we will add users later.
<br/><br/>
...
* <code>Authentication Client</code>
<blockquote>
...
<br/><br/>
This package used [[https://github.com/restsharp/RestSharp|RestSharp]] rest clients and request to communicate with other services which needs authentication.
<br/><br/>
Authenticator is a tool just for getting tokens from auth server.
<br/><br/>
There is an implementation which get __clientId__ and __clientSecret__ and do the jobs for authentication with an Oauth2 server. IRestRequest from [[https://github.com/restsharp/RestSharp|RestSharp]] package is parameters for RestClient.
<br/><br/>
**This package is ready to use by Dependency injector containers
<br/><br/>

__How to implement client authenticator__<br/>
<blockquote>
    ...
    public sealed class Authenticator : IDisposable
    {
         static TokenResponse GetClientToken()
         {
                 var client = new TokenClient(
                     address: "http://localhost:port/token",
                     clientId: "tester",
...
* <code>Sample.AuthenticationServer</code>
<blockquote>
This package contains tools for validating request in [[http://owin.org|Owin]]. There is a middle ware that do the jobs for this. In this part we will create a middleware to add it at all web APIs that is configured to require an access token from the IdentityServer we just set up.<br/>
In the other words, This project has a extension class as __[[https://identityserver.github.io/Documentation/docsv2/consuming/overview.html|Katana Access Token Validation Middleware]]__
<br/><br/>
...
<br/><br/>
You can get the middleware here: [[https://www.nuget.org/packages/IdentityServer3.AccessTokenValidation|nuget]] or [[https://github.com/IdentityServer/IdentityServer3.AccessTokenValidation|source code]]. Necessary nuget packages for this middleware project's:
<br/><br/>
...
The middleware will first inspect the token - if it is a [[https://jwt.io|JWT]], token validation will be done locally (using the issuer name and key material found in the discovery document). If the token is a reference token, the middleware will use the access token validation [[https://identityserver.github.io/Documentation/docsv2/endpoints/accessTokenValidation.html|endpoint]] on IdentityServer (or the [[https://identityserver.github.io/Documentation/docsv2/endpoints/introspection.html|introspection endpoint]] is credentials are configured).
<br/><br/>
So, when an API want to use another API in secure level, should sent self __Access Token__ to that. Then token receiver API's validate the **Access Token** by this middleware without sending token to main authentication server to validate the token is true or not.
<br/><br/>
High level features: 
...
<br/><br/>
<blockquote>
library `system.identity.tokens.jwt` should never be updated. We believe the version 5.0 is incompatible with us services or has some bugs!
</blockquote>
